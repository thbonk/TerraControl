...
class Solar {
  ...
  class func sunRiseAndSet(
       for date: Date,
    at location: Location,
    in timeZone: TimeZone) throws -> (sunrise: Date, sunset: Date) {

    var calendar = Calendar(identifier: .gregorian)
    calendar.timeZone = timeZone
    var comp = calendar.dateComponents([.day, .year, .month], from: date)
    comp.calendar = calendar

    guard let year = comp.year,
          let month = comp.month,
          let day = comp.day else {
      throw SolarError.error(message: "Failed to find date components.")
    }

    let (riseUTC, setUTC, code) = Solar.sun_rise_set(year: year,
                                                       month: month,
                                                       day: day,
    ...
    if code != .RiseAndSet {
      throw SolarError.error(message: "Failed to find rise and set.")
    }

    // Calc sunrise
    var riseLocalHrs = riseUTC + (Double(timeZone.secondsFromGMT()) / 3600.0)
    ...
    guard let riseTime = comp.date else {
      throw SolarError.error(message: "Failed to calculate rise time hrs >\(riseLocalHrs)< from date components >\(comp)<")
    }

    // Calc sunset
    var setLocalHrs = setUTC + (Double(timeZone.secondsFromGMT()) / 3600.0)
    if setLocalHrs > 24.0 {
      setLocalHrs -= 24.0
    }
    ...
    guard let setTime = comp.date else {
      throw SolarError.error(message: "Failed to calculate set time hrs >\(setLocalHrs)< from date components >\(comp)<")
    }
  ...
  class func noon(
    ...
    let sun = try sunRiseAndSet(for: date, at: location, in: timeZone)
    let sunTime = sun.sunset.timeIntervalSince(sun.sunrise)
    let noon = sun.sunrise.addingTimeInterval(sunTime / 2)
